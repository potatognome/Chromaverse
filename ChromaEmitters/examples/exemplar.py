#!/usr/bin/env python3
"""
examples/exemplar.py - ChromaEmitters exemplar mock entry point.
"""
...
def _log_targets() -> list[str]:
    ...
    if not out:
        out = [str(log_root / "ChromaEmitters_SESSION.log"), str(log_root / "ChromaEmitters_MASTER.log")]
    return out
...
def run_module_demo() -> None:
    ...
    terminal_settings = emitters.get("terminal", {})
    tapo_settings = emitters.get("tapo_led", {})

    log_line(f"terminal settings loaded: {bool(terminal_settings)}", key="!done")
    log_line(f"tapo settings loaded: {bool(tapo_settings)}", key="!done")

    sample_frame = {"h": 0.1, "s": 0.8, "v": 0.9}
    if bool(terminal_settings.get("enabled", True)):
        log_line(f"terminal render sample: {{'emitted': True, 'frame': {sample_frame}}}", key="!data")
    else:
        log_line("terminal emitter is disabled by config", key="!warning")
...
def run_edge_cases() -> None:
    ...
    emitters = CONFIG.get("emitters", {}) if isinstance(CONFIG.get("emitters", {}), dict) else {}
    missing = emitters.get("missing-emitter", {})
    log_line(f"missing emitter settings: {missing}", key="!data")
